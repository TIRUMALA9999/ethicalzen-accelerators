...
func main() {
	...
	if cfg.Gateway.Mode == "local" {
		...
		log.Info("🏠 [LOCAL MODE] Skipping Redis initialization (not required)")
		cacheClient = nil
	} else if !cfg.Cache.Enabled {
		// Cache is disabled in config
		log.Info("ℹ️  Cache disabled in configuration, running without Redis")
		cacheClient = nil
	} else {
	...
	if cfg.Gateway.Mode != "local" && cfg.Blockchain.Enabled {
		...
		log.Info("✅ Blockchain client initialized")
	} else {
		if cfg.Gateway.Mode == "local" {
			log.Info("🏠 [LOCAL MODE] Skipping blockchain client (not required)")
		} else {
			log.Info("ℹ️  Blockchain disabled in configuration")
		}
	...
	if blockchainClient != nil {
		if err := blockchainClient.HealthCheck(ctx); err != nil {
			...
			log.Info("✅ Blockchain connection healthy")
		}
	}
	...
	log.Info("✅ Proxy handler initialized")
	...
	if cacheClient != nil {
		...
		log.Info("✅ API handler initialized (local mode with authentication)")
	}
	...
	}).Info("🔔 Registering webhook with backend...")

	if err := gateway.RegisterWebhook(backendURL, apiKey, webhookURL, gatewayID); err != nil {
		log.WithError(err).Warn("⚠️  Failed to register webhook, falling back to polling")
	} else {
		log.Info("✅ Webhook registered successfully - instant contract sync enabled!")
	}
	...
	if localMode {
		...
		log.Info("🏠 [LOCAL MODE] Gateway running in local development mode")
		...
		if cfg.Gateway.APIKey != "" && cfg.Gateway.TenantID != "" && cfg.Gateway.ControlPlaneURL != "" {
			...
			}).Info("📡 [LOCAL MODE] Control plane credentials found, registering gateway...")
			...
			if err != nil {
				log.Warnf("❌ [LOCAL MODE] Gateway registration failed: %v", err)
				log.Info("⚠️  [LOCAL MODE] Trying direct contract sync with customer API key...")
				...
				if err := gateway.LoadContractsFromControlPlane(ctx, cfg.Gateway.ControlPlaneURL, cfg.Gateway.APIKey, cfg.Gateway.TenantID); err != nil {
					log.Warnf("❌ [LOCAL MODE] Failed to load contracts: %v", err)
					log.Info("⚠️  [LOCAL MODE] Gateway will start but may not have any contracts loaded")
				}
			} else {
				log.Info("✅ [LOCAL MODE] Gateway registered successfully, loading contracts...")
				...
				if err := gateway.LoadContractsFromControlPlane(ctx, cfg.Gateway.ControlPlaneURL, gatewayAPIKey, cfg.Gateway.TenantID); err != nil {
					log.Warnf("❌ [LOCAL MODE] Failed to load contracts with gateway API key: %v", err)
					log.Info("⚠️  [LOCAL MODE] Gateway will start but may not have any contracts loaded")
				} else {
					log.Info("✅ [LOCAL MODE] Contracts loaded from control plane")
			...
			log.Info("🏠 [LOCAL MODE] No control plane credentials - running in standalone mode")
			log.Info("💡 [LOCAL MODE] Contracts will be loaded in-memory on demand")
			log.Info("💡 [LOCAL MODE] To sync with backend, set ETHICALZEN_API_KEY, GATEWAY_TENANT_ID, and CONTROL_PLANE_URL")
		}

	} else {
		// CLOUD MODE: Load contracts from Redis (multi-tenant)
		log.Info("☁️  [CLOUD MODE] Gateway running in cloud multi-tenant mode")
		...
		if cacheClient != nil {
			...
			if err := gateway.LoadAllContractsAtBoot(ctx, cacheClient); err != nil {
				...
				log.Info("✅ Contracts loaded for feature extraction")
			}
			...
			log.Info("ℹ️  [CLOUD MODE] Cache disabled, using in-memory store")
			...
			if backendURL == "" {
				log.Warn("⚠️  No backend URL configured, contracts will not be loaded")
			} else {
				...
				log.WithField("backend_url", backendURL).Info("📡 Syncing contracts from backend API...")
				if err := gateway.SyncContractsFromBackend(ctx, backendURL, apiKey); err != nil {
					log.WithError(err).Warn("⚠️  Failed to sync contracts from backend")
					log.Warn("   Gateway will start but validation may fail until contracts are loaded")
				} else {
					log.Info("✅ Contracts synced from backend API")
				}
	...
	//   → Stable, versioned with gateway releases
	//   → Available to ALL tenants automatically
	//
	// - Tenant extractors: Custom domain-specific guardrails (loaded from GitHub)
	//   → Created via GDK in portal
	//   → Stored in tenant's own GitHub repo
	//   → Loaded dynamically at gateway boot
	//   → BYOG (Bring Your Own Guardrails) architecture
	//
	// This enables:
	...
	log.Info("🔧 Loading guardrails (Platform Bundled + Tenant GitHub)...")
	...
	log.WithField("path", guardrailRepoPath).Info("📦 STEP 1: Loading platform baseline guardrails (bundled)...")
	...
	if err := txrepo.LoadGuardrailsFromRepository(guardrailRepoConfig); err != nil {
		log.Errorf("❌ Failed to load platform guardrails from bundled filesystem: %v", err)
		log.Error("   Platform baseline security is unavailable!")
	} else {
		log.Info("✅ Platform baseline guardrails loaded from bundled image")
	}
	...
	log.Info("📦 STEP 2: Loading tenant-specific guardrails...")
	...
	if err := txrepo.LoadAllTenantGuardrails(); err != nil {
		log.Warnf("⚠️  No tenant guardrails loaded (tenants need to configure GitHub repos in portal): %v", err)
	} else {
		log.Info("✅ Tenant-specific guardrails loaded")
	}
	...
	// ============================================================================
	if cfg.Gateway.Mode == "local" {
		log.Info("🌐 [LOCAL MODE] Gateway starting in local mode...")
		...
		if cfg.Gateway.APIKey == "" {
			log.Fatal("❌ [LOCAL MODE] Missing API key. Set ETHICALZEN_API_KEY or gateway.api_key in config")
		}

		if cfg.Gateway.ControlPlaneURL == "" {
			log.Fatal("❌ [LOCAL MODE] Missing control plane URL. Set CONTROL_PLANE_URL or gateway.control_plane_url in config")
		}

		log.WithFields(log.Fields{
			"control_plane_url": cfg.Gateway.ControlPlaneURL,
			"tenant_id":         cfg.Gateway.TenantID,
		}).Info("🔐 [LOCAL MODE] Authenticating with control plane...")
		...
		if err != nil {
			log.Warnf("⚠️  [LOCAL MODE] Gateway registration failed: %v", err)
			...
			log.Info("✅ [LOCAL MODE] Gateway registered successfully")
		...
		// ============================================================================
		log.Info("☁️  [CLOUD MODE] Gateway starting in cloud mode...")

		if cacheClient != nil {
			// Redis enabled - load from Redis
			log.Info("📊 Loading contracts from Redis cache...")

			// Load all contracts at boot
			if err := gateway.LoadAllContractsAtBoot(ctx, cacheClient); err != nil {
				log.Warnf("⚠️  Failed to load contracts at boot: %v", err)
			...
			log.Info("ℹ️  Cache disabled, syncing contracts from backend API...")
			...
			if backendURL != "" {
				...
				log.Warn("⚠️  No backend URL configured for contract sync")
			}
		}
	}
	...
	if localMode {
		log.Info("📋 STEP 3: Loading mock contracts for local testing...")
		...
		if err := gateway.LoadMockContracts(ctx, mockContractsPath); err != nil {
			log.Warnf("⚠️  Failed to load mock contracts: %v", err)
			log.Warn("   Validation will be skipped for requests without loaded contracts")
		} else {
			log.Info("✅ Mock contracts loaded for local testing")
		}
	}
	...
	if cfg.Metrics.Enabled {
		...
		log.WithField("port", cfg.Gateway.MetricsPort).Info("✅ Metrics server configured")
	}
	...
	log.Info("👋 ACVPS Gateway stopped")
}
